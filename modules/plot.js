...
function plot2d_mlp_train(dataset, fn, fn_final, y_trans) {
  ...
  let domain = {
    x: [Math.min(...dataset.train.map((v) => {return v[0]})) - 0.5, Math.max(...dataset.train.map((v) => {return v[0]})) + 0.5],
    y: [Math.min(...dataset.train.map((v) => {return v[1]})) - 0.5, Math.max(...dataset.train.map((v) => {return v[1]})) + 0.5]
  };
  if (fn != null && fn_final != null && y_trans != null)
    domain = {
      x: [-0.5, 1.5],
      y: [-0.5, 1.5]
    }
...
function plot2d_mlp_test(dataset, fn, fn_final, y_trans) {
  ...
  let domain = {
    x: [Math.min(...dataset.test.map((v) => {return v[0]})) - 0.5, Math.max(...dataset.test.map((v) => {return v[0]})) + 0.5],
    y: [Math.min(...dataset.test.map((v) => {return v[1]})) - 0.5, Math.max(...dataset.test.map((v) => {return v[1]})) + 0.5]
  };
  if (fn != null && fn_final != null && y_trans != null)
    domain = {
      x: [-0.5, 1.5],
